    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todoapp/signupuser.html', {'form': UserCreationForm(), 'error': 'That username has already been taken'})    
        else:
            #Tell  pass1 != pass2 
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'todoapp/loginuser.html', {'form': AuthenticationForm(), 'error': 'username and pass did not match'})
        else:
            login(request, user)
            return redirect('currenttodos')
...
def createtodo(request):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'todoapp/createtodo.html', {'form': TodoForm(), 'error': 'Check your wrong data '})
